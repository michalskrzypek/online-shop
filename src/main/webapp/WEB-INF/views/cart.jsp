...
<strong>${message }</strong>
	...
	<c:choose>
				...
				<tbody>
					...
					<c:forEach items="${cartlines }" var="cartline">
						<tr>
							...
							<td data-th="Quantity"><input type="number" id="count_${cartline.getId() }" min=1 max="${cartline.getProduct().getQuantity()}"
							...
							<%-- <form action="${pageContext.request.contextPath }/cart/update/cartline/ }"> --%>
								<button type="submit" name="refreshButton" value="${cartline.getId()}" class="btn btn-info btn-sm">
									<span class=" glyphicon glyphicon-refresh"></span>
								</button>
							
							<form action="${pageContext.request.contextPath }/cart/delete/cartline/${cartline.getId() }" method="post">
		...
		<c:otherwise>
			...
			<a href="${pageContext.request.contextPath }/show/all/products" class="btn btn-warning"><span
